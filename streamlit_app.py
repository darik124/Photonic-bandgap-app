...
def backend_ok(url: str) -> bool:
    if not url:
        return False
@@ -13,66 +13,78 @@ def backend_ok(url: str) -> bool:
    except Exception:
        return False
...
if st.session_state["tx_data"]:
    ...
    freq = d.get("freq_GHz") or d.get("frequency_GHz") or []
    trdb = d.get("trans_dB") or d.get("transmission_dB") or []

    fig, ax = plt.subplots()
    ax.plot(freq, trdb, lw=1.6)
...
@@ -148,35 +170,40 @@ def backend_ok(url: str) -> bool:
